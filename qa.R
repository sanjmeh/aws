...
extract_zip <- function(zippath,opath = "~/Downloads/data3k_backup/"){
  unzip(zipfile = zippath,overwrite = T, setTimes = F,exdir = opath)
}

gen_csv <- function(ipath,tryfiles = NULL,isaddress=F){
  files <- list.files(ipath,full.names = T) 
  if(!is.null(tryfiles)){
  ...
  if(length(files) > 0){
    ...
    files <- files[(which(lengths(x1)>0))]
    x1 <-  x1[(which(lengths(x1)>0))]
